...
| Numeric procedures           | The rounding family `floor` / `ceiling` / `round` / `truncate`, the truncating- and flooring-division family `floor/` / `floor-quotient` / `floor-remainder` / `truncate/` / `truncate-quotient` / `truncate-remainder`, `square`, `exact-integer-sqrt`, `exact-integer?`, `exact-rational?`, `number->string`, and `string->number` are not defined. Only the r5rs-style `quotient` / `remainder` / `modulo` ship for integer division. Inexact reals are double-precision only. |
