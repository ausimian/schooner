...
| Numeric tower                | Integer, exact rational (`1/2`), double-precision float, and rectangular complex (`3+4i`, `+i`, `1@0.5`). Complex components may be any non-complex number — exact, inexact, or special. Prefixed-radix complex literals (`#e3+4i`) are not parsed; use the unprefixed form. |
