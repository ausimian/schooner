...
| Numeric tower                | Integer, exact rational (`1/2`), double-precision float, and rectangular complex (`3+4i`, `+i`, `1@0.5`). Complex components may be any non-complex number — exact, inexact, or special. `sqrt` of a non-square exact *positive* value raises rather than widening to inexact — call `(exact->inexact x)` first when an inexact root is wanted. `numerator` / `denominator` of a non-finite float (`+inf.0`, `+nan.0`) raises rather than returning a non-finite result. Real `(asin x)` / `(acos x)` outside `[-1, 1]` still raises rather than lifting silently — pass `(make-rectangular x 0)` to opt in. Prefixed-radix complex literals (`#e3+4i`) are not parsed; use the unprefixed form. |
