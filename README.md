...
| Numeric tower                | Integer, exact rational (`1/2`), and double-precision float. No complex (`3+4i`). `sqrt` of a non-square exact value raises rather than widening to inexact — call `(exact->inexact x)` first when an inexact root is wanted. `numerator` / `denominator` of a non-finite float (`+inf.0`, `+nan.0`) raises rather than returning a non-finite result. |
